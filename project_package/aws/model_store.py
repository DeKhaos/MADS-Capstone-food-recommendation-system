...
S3_BUCKET = os.getenv("S3_BUCKET", "us-east-1")
MODEL_PREFIX = os.getenv("MODEL_PREFIX", "us-east-1")
...
def upload_csv_file(
    ...
    """
    Upload a CSV file to S3 using the same conventions as other upload utilities.
    """
    path = Path(local_path).resolve()
    if not path.exists():
        raise FileNotFoundError(f"Local CSV file does not exist: {path}")
...
def load_csv_dataframe(
    ...
    **read_csv_kwargs,
    ...
    """
    Load a CSV file stored in S3 directly into a pandas DataFrame.
    """
    final_key = normalize_model_key(s3_key)
...
def upload_rectools_model(s3_client, model, model_name, bucket=S3_BUCKET):
    # Save rectools model into a buffer
    buffer = io.BytesIO()
    model.save(buffer)
    buffer.seek(0)
